...
export default function TeamPerformance({ data }) {
    ...
    const { workPatterns } = data;

    // Calculate max value for heatmap scaling
    const maxHeatmapValue = workPatterns ? Math.max(...workPatterns.heatmap.map(d => d.value)) : 0;

    const getHeatmapColor = (value) => {
        if (value === 0) return 'bg-dark-border/30';
        const ratio = value / (maxHeatmapValue || 1);
        // Using Primary color shades (assuming blue-ish primary)
        // Tailwind colors need to be safelisted or full strings.
        // We'll use inline styles for opacity to be safer with dynamic values or explicit classes
        // Let's use explicit classes for simplicity if possible, or style.
        // Tailwind doesn't support dynamic class interpolation unless configured.
        // We'll use style background.
        const opacity = 0.2 + (ratio * 0.8);
        return `rgba(51, 154, 240, ${opacity})`; // #339af0 is primary
    };

    return (
        <div className="space-y-6">
             <h2 className="text-xl font-bold text-white">Team Performance & Patterns</h2>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div className="h-64 flex flex-col">
                        ...
                        <div className="flex-1 grid grid-cols-[auto_1fr] gap-2">
                             ...
                             <div className="grid grid-cols-24 grid-rows-7 gap-1">
                                 {workPatterns && workPatterns.heatmap.map((cell, i) => (
                                     <div
                                        key={i}
                ...
                <Card title="Activity by Hour of Day" icon={Clock}>
                     <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={workPatterns ? workPatterns.hourlyActivity : []}>
